...
for i, row in link_df.iterrows():
    ...
    for curr_contour_file in os.listdir(i_contour_dir):
        ...
        if contour_lst:
            ...
            if contour_id in curr_dicom_dict:
                ...
                #####logging.warning("Missing matching ")
                continue

            # save to dict
            curr_contour_dict[contour_id] = poly_to_mask(contour_lst, *dims)
    ...
    for curr_missing in missing_contours:
        logging.warning("Missing the i-contour file: patient {}, contour {}".\
                        format(row["patient_id"], curr_missing))

    # get intersection of files so we know what to interate over
    intersection_lst = list(set(curr_contour_dict.keys())&set(curr_dicom_dict.keys()))
    for curr_k in intersection_lst:
        ...
        # write contour mask overlaid on image
